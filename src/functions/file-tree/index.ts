...
export async function fileTreeGenerator(uri?: unknown): Promise<void> {
  try {
    if (typeof uri !== 'object' || uri === null || !('fsPath' in uri) || typeof uri.fsPath !== 'string') {
    ...
    // 复制到剪贴板。必须 await：未 await 的 rejection 进不了下面的 catch，
    // 会变成 unhandled rejection，且成功提示会早于写入完成弹出。
    await env.clipboard.writeText(content);

    // 显示成功提示
    window.setStatusBarMessage('The file tree has been copied to the clipboard :)', 4000);
  } catch (error) {
    // 错误处理
